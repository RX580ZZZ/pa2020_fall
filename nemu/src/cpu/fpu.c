...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs)
{
	...
	if (!overflow)
	{
		...
		uint32_t tmp = sig_grs & 0x7;
		sig_grs = sig_grs >> 3;
		if (tmp > 4 || (tmp == 4 && (sig_grs & 0x1) == 1)){
		    sig_grs += 1;
		}
		/*if ((sig_grs >> 23) == 1 && exp == 0) exp++;*/
...
uint32_t internal_float_add(uint32_t b, uint32_t a)
{
	...
	uint32_t shift = 0; //fb.exponent - fa.exponent wrong,
	...
	//if (b == 0xbf000000 && a == 1) printf("%x %x %x", f.sign, exp_res, sig_res);
...
uint32_t internal_float_div(uint32_t b, uint32_t a)
{
	...
	//uint32_t exp_res = fa.exponent + 127 + 26 - shift - fb.exponent ;
	return internal_normalize(f.sign, exp_res, sig_res);
}
